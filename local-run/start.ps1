# Golem Services Start Script for Windows
# PowerShell equivalent of start.sh

# Clean up any existing data
if (Test-Path ".\local-run\data\shard-manager") { Remove-Item -Recurse -Force ".\local-run\data\shard-manager" }

# Create directories
New-Item -ItemType Directory -Path ".\local-run\data\redis", ".\local-run\data\shard-manager", ".\local-run\logs" -Force | Out-Null
...
Write-Host "Starting Golem services on Windows..."
...
try {
    ...
    Write-Host "`nAll services are running. Press Ctrl+C to stop all services."
    Write-Host "Cloud Service UI: http://localhost:$CLOUD_SERVICE_HTTP_PORT"
    Write-Host "Worker Service UI: http://localhost:$WORKER_SERVICE_HTTP_PORT"
